...
- LMS at https://staging.tmp.global-communications-academy.com
- CMS at https://studio.staging.tmp.global-communications-academy.com
- CDN at https://cdn.staging.tmp.global-communications-academy.com linked to a public read-only S3 bucket named staging-codlp-global-storage
- Grafana at https://grafana.staging.tmp.global-communications-academy.com/login
  - user: admin
  - pwd: prom-operator
- public ssh access via a t2.micro Ubuntu 20.04 LTS bastion EC2 instance at bastion.staging.tmp.global-communications-academy.com
...
- LMS at https://dev.staging.tmp.global-communications-academy.com
- CMS at https://studio.dev.staging-global-communications-academy.com
- CDN at https://cdn.dev.staging.tmp.global-communications-academy.com linked to an S3 bucket named dev-codlp-global-storage
...
.. code-block:: shell
  ...
  ssh bastion.staging.tmp.global-communications-academy.com -i path/to/codlp-global-live-bastion.pem

  mysql -h mysql.staging.tmp.global-communications-academy.com -u root -p

  mongo --port 27017 --host mongo.master.staging.tmp.global-communications-academy.com -u root -p
  mongo --port 27017 --host mongo.reader.staging.tmp.global-communications-academy.com -u root -p

  redis-cli -h redis.primary.staging.tmp.global-communications-academy.com -p 6379
...
- **MySQL**. uses `AWS RDS <https://aws.amazon.com/rds/>`_ for all MySQL data, accessible inside the vpc as mysql.staging.tmp.global-communications-academy.com:3306. Instance size settings are located in the `environment configuration file <terraform/environments/staging/env.hcl>`_, and other common configuration settings `are located here <terraform/environments/staging/rds/terragrunt.hcl>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
- **MongoDB**. uses `AWS DocumentDB <https://aws.amazon.com/documentdb/>`_ for all MongoDB data, accessible insid the vpc as mongodb.master.staging.tmp.global-communications-academy.com:27017 and mongodb.reader.staging.tmp.global-communications-academy.com. Instance size settings are located in the `environment configuration file <terraform/environments/staging/env.hcl>`_, and other common configuration settings `are located here <terraform/modules/documentdb>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
- **Redis**. uses `AWS ElastiCache <https://aws.amazon.com/elasticache/>`_ for all Django application caches, accessible inside the vpc as cache.staging.tmp.global-communications-academy.com. Instance size settings are located in the `environment configuration file <terraform/environments/staging/env.hcl>`_. This is necessary in order to make the Open edX application layer completely ephemeral. Most importantly, user's login session tokens are persisted in Redis and so these need to be accessible to all app containers from a single Redis cache. Common configuration settings `are located here <terraform/environments/staging/redis/terragrunt.hcl>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
...
- **CDN**. uses `AWS Cloudfront <https://aws.amazon.com/cloudfront/>`_ as a CDN, publicly acccessible as https://cdn.staging.tmp.global-communications-academy.com. Terraform creates Cloudfront distributions for each of your enviornments. These are linked to the respective public-facing S3 Bucket for each environment, and the requisite SSL/TLS ACM-issued certificate is linked. Terraform also automatically creates all Route53 DNS records of form cdn.staging.tmp.global-communications-academy.com. Common configuration settings `are located here <terraform/environments/staging/cloudfront/terragrunt.hcl>`_
